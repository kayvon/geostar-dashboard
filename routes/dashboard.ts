...
// Helper to convert date string to Unix ms (start of day)
function dateToUnixMs(dateStr: string, endOfDay = false): number {
...
app.get('/', async (c) => {
  ...
  const fromMs = dateToUnixMs(date_from);
  const toMs = dateToUnixMs(date_to, true);
...
app.get('/daily', async (c) => {
  ...
  const fromMs = dateToUnixMs(date);
  const toMs = dateToUnixMs(date, true);
...
app.get('/readings', async (c) => {
  ...
  if (date_from) {
    ...
    params.push(dateToUnixMs(date_from));
  }
  if (date_to) {
    conditions.push('timestamp <= ?');
    params.push(dateToUnixMs(date_to, true));
  }
